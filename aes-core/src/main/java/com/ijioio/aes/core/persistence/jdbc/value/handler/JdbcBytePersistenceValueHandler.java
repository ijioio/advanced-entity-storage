...
public class JdbcBytePersistenceValueHandler extends BaseJdbcPersistenceValueHandler<Byte> {
	...
	public void write(JdbcPersistenceContext context, JdbcPersistenceHandler handler, TypeReference<Byte> type,
		...
		try {
			statement.setObject(context.getNextIndex(), value, Types.TINYINT);
	...
	public void writeCollection(JdbcPersistenceContext context, JdbcPersistenceHandler handler,
		...
		try {
			...
			Array array = values != null
					? statement.getConnection().createArrayOf(JDBCType.valueOf(Types.TINYINT).getName(),
							values.toArray())
					: null;
	...
	public Collection<Byte> readCollection(JdbcPersistenceContext context, JdbcPersistenceHandler handler,
		...
		try {
			...
			if (array != null) {
				...
				collection.clear();
				collection.addAll(Arrays.stream((Object[]) array.getArray()).map(item -> (Byte) item)
