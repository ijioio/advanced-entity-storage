...
public class JdbcLongPersistenceValueHandler extends BaseJdbcPersistenceValueHandler<Long> {
	...
	public void write(JdbcPersistenceContext context, JdbcPersistenceHandler handler, TypeReference<Long> type,
		...
		try {
			statement.setObject(context.getNextIndex(), value, Types.BIGINT);
	...
	public void writeCollection(JdbcPersistenceContext context, JdbcPersistenceHandler handler,
			TypeReference<? extends Collection<Long>> type, Collection<Long> values, boolean search)
		...
		try {
			...
			Array array = values != null
					? statement.getConnection().createArrayOf(JDBCType.valueOf(Types.BIGINT).getName(),
							values.toArray())
					: null;
	...
	@Override
	public Collection<Long> readCollection(JdbcPersistenceContext context, JdbcPersistenceHandler handler,
			TypeReference<? extends Collection<Long>> type, Collection<Long> values) throws PersistenceException {
		...
		try {
			...
			if (array != null) {
				...
				Collection<Long> collection = getCollection(type, values);

				collection.clear();
				collection.addAll(Arrays.stream((Object[]) array.getArray()).map(item -> (Long) item)
