...
public abstract class BaseDeleteEntityStorageTest<E extends Entity, I extends EntityIndex<E>>
	...
	public void before() throws Exception {
		...
		persistenceHandler = new H2PersistenceHandler(dataSource);
	...
	public void testDelete() throws Exception {
		...
		for (int i = 0; i < entityContainers.size(); i++) {
			...
			entityStorage.delete(entityContainer);

			Assertions.assertNull(entityStorage.load(entityContainer.toReference()));
			...
			expectedIndexes = createIndexes(entityContainers.subList(i + 1, entityContainers.size()));
			actualIndexes = entityStorage.search(
					SearchQueryBuilder.of(getIndexClass()).sorting(BaseEntityIndex.Properties.id, Order.ASC).build());
